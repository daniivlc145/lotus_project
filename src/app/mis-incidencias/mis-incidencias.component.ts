...
private crearNuevoElemento(elem: {[clave:string]:string}): void {
  ...
  nuevoSVG.setAttribute('style', 'width:300px; position:relative; z-index: 1; filter: drop-shadow(0 0 10px rgba(0, 0, 0, 0.15));');
  ...
  labelUbi.setAttribute('style',' font-size: 16px;  text-align: left;color: #3a5e62; position: relative;z-index: 6; margin-bottom:20%')
  nuevoDiv.appendChild(labelUbi);

  const labeldesc = document.createElement('p');
  labeldesc.className = 'label-desc';
  labeldesc.textContent='Descripción:';
  labeldesc.setAttribute('style',' font-size: 16px;  text-align: left;color: #3a5e62; position: relative;z-index: 6; margin-bottom:20%')
  nuevoDiv.appendChild(labeldesc);

  const fechaElement = document.createElement('p');
  fechaElement.className = 'texto-DATE';
  fechaElement.textContent = elem['fecha'];
  fechaElement.setAttribute('style',' font-size: 20px;  text-align: left;color: #ffffff; position: relative;z-index: 6; margin-bottom:20%')
  nuevoDiv.appendChild(fechaElement);

  // Crear elementos para cada dato y agregarlos al nuevo div
  const tipoElement = document.createElement('p');
  tipoElement.className = 'texto-TIPO';
  tipoElement.textContent = elem['type'];
  tipoElement.setAttribute('style',' font-size: 25px;  text-align: left;color: #ffffff; position: relative;z-index: 6; margin-bottom:20%')
  nuevoDiv.appendChild(tipoElement);

  const horaElement = document.createElement('p');
  horaElement.className = 'texto-TIME';
  horaElement.textContent = elem['hora'];
  horaElement.setAttribute('style',' font-size: 20px;  text-align: left;color: #ffffff; position: relative;z-index: 6; margin-bottom:20%')
  nuevoDiv.appendChild(horaElement);

  const descripcionElement = document.createElement('p');
  descripcionElement.className = 'texto-desc';
  descripcionElement.textContent = elem['descripcion'];
  descripcionElement.setAttribute('style',' font-size: 14px;  text-align: left;color: #b5b4b8; position: relative;z-index: 6; margin-bottom:20%')
  nuevoDiv.appendChild(descripcionElement);
  ...
  geoShapeElement.setAttribute('style',' font-size: 14px;  text-align: left;color: #b5b4b8; position: relative;z-index: 6; margin-bottom:20%')
  nuevoDiv.appendChild(geoShapeElement);
